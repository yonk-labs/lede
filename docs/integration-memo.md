# Integration Memo — lede in chunkshop

**Status:** Design agreed; runtime before/after pending chunkshop Brief 4 implementation.
**Date:** 2026-04-26 (refreshed for lede v0.2.0).
**SC-009 evidence:** Partial — design/contract complete, quantitative before/after deferred until consumer code ships.

> **v0.2 update:** the four-function contract below remains supported and byte-stable; v0.2 layered on a richer `summarize(attach=…) -> SummaryResult` shape plus the `lede.extract.*` namespace and the `lede.brief()` composer. chunkshop wrappers can adopt the new shape incrementally — see [Adopting the v0.2 RAG-prep API](#adopting-the-v02-rag-prep-api) below.

This memo documents how `lede` integrates into [`chunkshop`](https://github.com/yonk-labs/chunkshop) as its first real downstream consumer. It is deliberately **API-contract-focused**: there is **zero chunkshop-specific code in lede**, and zero lede-specific code in chunkshop. Integration lives entirely in chunkshop's wrappers.
...
Before this design, chunkshop had exactly one extractor (`rake_keywords`) and no concept of summarization in the pipeline. Summarization wasn't "replaced" — it's a **new capability** chunkshop gains by consuming lede. For retrieval, chunkshop previously embedded raw chunk text, which is noisier than embedding a focused summary.

## Integration architecture

lede's job is to expose four stable library functions. chunkshop's job is to wrap them in whatever pipeline shape it needs. Neither project names the other in its source.

| lede exposes                                 | chunkshop consumes via                                                                    | Reference brief (chunkshop-side)        |
|-----------------------------------------------|-------------------------------------------------------------------------------------------|-----------------------------------------|
...
The contract chunkshop extractors write to is `ExtractResult(tags: list[str], metadata: dict)`, introduced in the [schema-flexibility brief](https://github.com/yonk-labs/chunkshop/blob/main/skill-output/mission-brief/Mission-Brief-schema-flexibility.md). Future lede additions (e.g. structured metadata extraction from Thread C) flow through the same field without touching the contract shape.

## lede API contract

The four functions above are part of lede's stable public surface and have not changed signatures since the v0.0.1 reference. Their output bytes are deterministic and byte-identical across Python and Rust (`fixtures/` is the source of truth; `rust/tests/fixtures.rs` asserts parity on every CI push).
...
```rust
use lede::{summarize, extract_keyword, clean_text, strip_think};

lede::summarize(text: &str, max_length: usize, mode: lede::Mode) -> SummaryResult
lede::extract_keyword(text: &str, keywords: &str, num_sentences: usize) -> String
lede::clean_text(text: &str) -> String
lede::strip_think(text: &str) -> String
```
...
chunkshop wrappers may compose these freely. lede does not know chunkshop exists.
...
| lede v0.2 surface | chunkshop consumption (proposed/incremental) | Notes |
|---|---|---|
...
| `r.stats`, `r.metadata.dates`, `r.metadata.amounts`, `r.metadata.urls` | `metadata` field of `ExtractResult` — structured per-chunk facts without a second pass | replaces the proposed `lede.extract_metadata` shape from Thread C; the contract was already there |
| `r.metadata.entities` (requires `lede[ner]` extra) | `tags` field of `ExtractResult` (PERSON/ORG/GPE) | optional — Rust path leaves `entities` empty; chunkshop opts in via the extra |
...
| `lede.brief(chunk_text, format="dict")` | preview/citation generator for each chunk's coarse row in `hierarchical_summary` | one call yields overview + key facts + section names |
| `lede.extract.toc(chunk_text)` | section-aware chunking heuristic in semantic chunker | replaces a manual heading regex |
...
- **`extract_keyword`**: proposed extension of Brief 1 (metadata-extractors). A `callable_phrases` variant takes a user-supplied callable whose signature matches lede's `extract_keyword`, producing tags attached to each chunk. Keeps chunkshop origin-agnostic. (v0.2 alternative: `summarize(attach=["phrases"])` returns the same kind of thing in a single call.)
- **`clean_text` / `strip_think`**: proposed Brief 5 introduces `Source.preprocessors: list[Callable[[str], str]]` — a generic preprocessing chain. Users compose lede's functions (or their own) to scrub filler/markdown/`<think>` blocks before chunking.

None of this requires chunkshop to depend on lede. The contracts accept any Python callable with matching signatures. lede happens to be the deterministic zero-dep implementation that ships with chunkshop's docs.
...
1. **Zero lede changes needed.** The brainstorm expected lede would need an "integration mode" or chunkshop-shaped adapter. It doesn't. lede's v0.1.0 API surface is already the right shape; chunkshop just wraps it.
...
3. **P4 (lede-owns-chunker) is a non-goal.** Chunking is chunkshop's concern. lede stays out of boundary decisions.
4. **Source preprocessors had no seam** in chunkshop today — users had to pre-clean their corpus manually. The cleanest fix is a generic `preprocessors` chain, not lede-specific plumbing. Motivates Brief 5.
5. **The `ExtractResult(tags, metadata)` contract absorbs future lede additions.** v0.2 confirmed this: `lede.extract.metadata(text) -> Metadata(dates, amounts, urls, entities)` and the rest of `lede.extract.*` all flow into `ExtractResult.metadata` / `.tags` without re-shaping. The original prediction held.

## Scope boundaries

- **lede does not depend on chunkshop.** lede is a library; chunkshop is one of many possible consumers.
- **chunkshop does not depend on lede.** chunkshop's contracts accept any callable with the right signature — lede, `lede-neural`, LLM APIs, or user code.
- **Neural summarization lives in the `lede-neural` companion**, not here. Same callable shape; different implementation.
...
- lede: [`README.md`](../README.md) · [`docs/REFERENCE.md`](REFERENCE.md) (v0.2 primitive catalog) · v0.2 design contract at [`docs/v0-2-design.md`](v0-2-design.md).
