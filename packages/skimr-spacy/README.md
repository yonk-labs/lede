...
for `skimr.extract.metadata`, `skimr.extract.phrases`, and
`skimr.extract.correlate_facts`.

| | regex backend (default in `skimr`) | spaCy backend (this package) |
|---|---|---|
...
skimr's regex backend covers the majority of structured-extract use cases for
...
skimr-spacy specifically when you need named entities** — people,
...
import skimr_spacy  # side effect: registers the 'spacy' backend
```
...
### `backend="regex"` (skimr default — zero-dep)
...
- Your callers want **PERSON / ORG / GPE entities** in the output. Default skimr can't help.
...
- You don't actually need entities. The default skimr regex backend already handles dates / amounts / URLs / numeric facts with sentence context.
- You're shipping skimr inside a constrained environment (Lambda cold-start, embedded, no-egress) — the 50 MB `en_core_web_sm` model has real cost.
...
Pulls `skimr`, `spacy>=3.8,<3.9`, and the pinned `en_core_web_sm` 3.8.0
...
import skimr
skimr.set_default_backend("auto")   # spaCy if registered, else regex
```
...
| Default skimr regex backend (for comparison) | <1 ms |

Run `from skimr_spacy import warmup; warmup()` at app startup to pay the
50 ms once instead of on the first user request.

## What's registered

When you `import skimr_spacy`, three backends register themselves into
`skimr.extract._backends`:

| skimr primitive | spaCy backend |
|---|---|
...
The `regex` backend stays the default — `import skimr_spacy` is purely
...
same entities for the same input on every call. **It is not on skimr's
...
[`docs/skimr-spacy-integration.md`](https://github.com/yonk-labs/skimr/blob/main/docs/skimr-spacy-integration.md)
for the cross-language policy.

If you need NER from a Rust service today: call out to a Python
skimr-spacy worker, or to a hosted NER endpoint. A future
`skimr-rust-ner` companion crate is on the roadmap if there's demand —
file an issue.

## License

Apache-2.0, same as skimr.
