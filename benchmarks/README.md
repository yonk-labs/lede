...
Scaffold for mission-brief **SC-006**: "fastest of {Python lede, Rust lede} within **2× of Sumy** wall-clock on an equivalent-algorithm benchmark." This directory is the harness; `results/` holds captured numbers.
...
├── harness.py            timing runner for lede (tfidf, textrank) + Sumy (LexRank, TextRank, LSA)
...
Rust lede columns are explicit `—` placeholders. Plan 2 fills them in.

## What it deliberately does not measure (yet)

- **Quality.** No ROUGE, no LLM-judge. Wall-clock + output-length only for now. Quality measurement lands once `lede-neural` forces the issue.
...
Cross-machine numbers are invalid. The harness captures a system fingerprint (CPU, cores, Python version, lede + Sumy versions) in every results file so historical comparisons stay honest. Pin the target machine if you need a stable baseline.

## Corpus notes

Five inputs, 475 to 3,105 chars, written to cover distinct document shapes:

- `crm-note-short.txt` — shorter than the default 500-char budget, so lede/tfidf short-circuits (passthrough). Useful for confirming the fast-path.
...
|---|---|
| `lede/tfidf` | < 0.5 ms for inputs up to ~3 KB |
| `lede/textrank` | 1–3 ms |
| `sumy/*` | 10–20 ms across backends |

lede/tfidf is roughly 50–70× faster than the fastest Sumy backend in the current harness. SC-006's "within 2× of Sumy" bar is already cleared by Python alone on this hardware; the Rust port has headroom to spare.
